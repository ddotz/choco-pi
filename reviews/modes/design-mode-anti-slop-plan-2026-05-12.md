...
### Current ddotz-pi state

- `modes/design/MODE.md` and `extensions/ddotz-autopilot/design-policy.ts` already require a design brief, existing design-system scan, visual thesis, responsive/accessibility states, and browser-backed QA.
- `extensions/ddotz-autopilot/mode-resource-policy.ts` currently names `frontend-ui-ux`, `taste-skill`, `web-design-guidelines`, and `gstack`, but does not name `taste-ko` or slide-specific skills.
...
- Pi extensions can enforce quality in `message_end` and queue hidden repair follow-ups; existing ddotz-pi guard architecture already supports this pattern.
...
- Decision: do not add it as a dependency. It is a workflow package, not a Korean UI typography/design-mode overlay, and would overlap ddotz-pi’s existing autonomy/guard system.
- Rejected scope: wholesale package adoption, vendoring its agents/skills, or replacing ddotz-pi mode flow.
- Idea-only note: its `frontend-design` skill reinforces style-system scan before aesthetic direction and explicit anti-slop aesthetic checkpoints. That idea matches existing ddotz-pi direction and is reflected as local policy language, not copied code.
...
- Modify: `extensions/ddotz-autopilot/mode-resource-policy.ts`
- Modify: `extensions/ddotz-autopilot/design-policy.ts`
...
- Create: `extensions/ddotz-autopilot/design-quality.ts`
...
- Modify: `README.md`
- Maybe modify: `package.json`, `extensions/ddotz-autopilot/version.ts`
...
3. Keep `package.json`, `extensions/ddotz-autopilot/version.ts`, and README current version synchronized.
