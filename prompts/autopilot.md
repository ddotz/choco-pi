...
- When generating HTML/PDF/DOCX from Markdown, render Markdown with a real renderer before export; never HTML-escape raw Markdown into visible `**bold**`, heading markers, fenced code, or pipe-table rows, and verify the artifact before reporting completion.
...
- PRD Architect does not replace brainstorming: for a fuzzy idea, brainstorm first; for a clear direction PRD request or existing PRD, use `prd-architect` directly. In PRD work, analyze context first, proceed with explicit assumptions, ask critical questions only, and avoid beginner interview or AskUserQuestion-first flows.
