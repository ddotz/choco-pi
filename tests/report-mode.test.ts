...
describe("report mode isolation", () => {
  ...
  it("adds evidence, layout, polishing, and confidence guardrails only in report mode", () => {
    ...
    expectContainsAll(prompt, [
      ...
      "Report mode must actively decide the smallest sufficient research scope before synthesis",
      "Avoid automatic broad web-analysis when the prompt can be answered from user-provided material or a bounded internal brief",
      "source confidence scoring",
      "evidence ledger is complete enough to support the key claims",
      "If external research is explicitly forbidden or actively deemed unnecessary, state the evidence boundary",
