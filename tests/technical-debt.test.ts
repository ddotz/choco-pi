...
import { isMajorTask, buildTechnicalDebtCleanupGuidance } from "../extensions/ddotz-autopilot/technical-debt";
...
describe("technical debt cleanup policy", () => {
  it("lets autopilot classify major tasks without asking the user", () => {
    ...
    expect(isMajorTask({ changedFiles: ["extensions/ddotz-autopilot/index.ts"], persistenceChanged: true })).toBe(true);
  });
