...
- Focus on external repo/link/package/upstream-change adoption decisions for choco-pi.
...
- Tracking decision: track only sources actually reflected into choco-pi or explicitly requested by the user. Use `source_registry.watch` when a source is relevant but not ready or safe to adopt.
