...
export function buildResponseStyleGuidance(): string {
  return [
    ...
    "Use confidence labels: High, Medium, Low. Do not use Korean labels like [높음], [중간], [낮음]. In terminal/UI contexts, render them as high-contrast badges: High with green background, Medium with yellow background, Low with red background, all with white text when possible.",
  ].join("\n");
}
