// Adapted from pi-btw 0.4.0 (https://github.com/dbachelder/pi-btw) under the MIT License.
// See THIRD_PARTY_NOTICES.md for the retained copyright and license notice.
...
export const BTW_SIDE_SESSION_TOOLS = ["read"] as const;
...
const BTW_SYSTEM_PROMPT = [
  "당신은 사용자의 메인 작업 세션과 분리된 별도 사이드 대화를 진행합니다.",
  "메인 세션 메시지가 제공되면 맥락 참고용으로만 사용하세요. 그 작업은 다른 에이전트가 처리 중입니다.",
  "메인 세션 메시지가 없으면 완전히 독립된 tangent thread로 간주하고 사용자의 말과 일반 지시만 따르세요.",
  "사용자의 부가 질문에 답하고, 아이디어를 정리하고, 다음 단계를 계획하는 데 집중하세요.",
...
const BTW_SUMMARIZE_SYSTEM_PROMPT =
  "별도 사이드 대화를 간결하게 요약하세요. 핵심 결정, 계획, 인사이트, 리스크, 액션 아이템을 보존하세요. 출력은 요약만 포함하고 한국어 존댓말을 사용하세요.";

const BTW_CONTINUE_THREAD_USER_TEXT = "[다음은 메인 세션과 분리된 별도 사이드 대화입니다. 이 thread를 이어서 진행하세요.]";
const BTW_CONTINUE_THREAD_ASSISTANT_TEXT = "알겠습니다. 별도 사이드 대화를 이어가겠습니다.";
...
function extractAnswer(message: AssistantMessage): string {
  return extractText(message.content, "text") || "(텍스트 응답 없음)";
}
    ...
    if (name === "btw:inject") {
      ...
      try {
        ...
        const content = instructions
          ? `별도 사이드 대화 내용입니다. ${instructions}\n\n${formatThread(thread)}`
          : `추가 맥락으로 참고할 별도 사이드 대화 내용입니다:\n\n${formatThread(thread)}`;
